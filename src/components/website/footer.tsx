  ...
  return (
    <footer className=" bg-secondary-950 py-20 mt-20 ">
      <div className=" max-w-screen-xl mx-auto px-4 md:px-0">
        <div className="grid grid-cols-1 md:grid-cols-2  xl:grid-cols-4 gap-8  text-secondary-100/90">
          <div className="max-w-xs">
            ...
            />
            <p className="leading-relaxed mt-2 text-[15px] tracking-wide"> Dont just take our word for it, come see us and let us show you that youve come to the right place.</p>
          </div>
          ...
          {footerNavs.map((item, idx) => (
            <ul
              className="space-y-4"
              key={idx}>
              <h4 className="text-secondary-50 font-medium text-lg">{item.label}</h4>
              {item.items.map((el: any, idx: number) => (
                <li key={idx}>
                  <Link
                    ...
                    {el.preicon} {el.name} {el.showpostIcon && <ChevronRight size={16} />}
                  </Link>
                </li>
              ))}
            </ul>
