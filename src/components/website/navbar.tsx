...
export default function Navbar() {
  ...
  const navigation = [
    { id: 0, title: "Abroad Study", path: "/", isDrapdown: true, navs: studyAbroads },
    { id: 1, title: "Test Preparation", path: "/", isDrapdown: true, navs: testPreparations },
        ...
        <div className="items-center gap-x-14 px-4 md:px-0 max-w-screen-xl mx-auto md:flex  ">
          <div className="flex items-center justify-between py-3 md:py-5 md:block ">
            ...
            <div className="md:hidden">
              <button
                type="button"
                className="text-secondary-100 hover:text-secondary-50"
                ...
                return (
                  <li key={idx}>
                    {item.isDrapdown ? (
                      <button
                        type="button"
                        className="w-full flex items-center justify-between gap-1 text-secondary-50 hover:text-primary-600 duration-300 "
                                ...
                                <Link
                                  ...
                                  <div  className=" hidden md:block">
                                    <div className=" bg-secondary-900 h-10 w-10 p-2 flex items-center justify-center rounded-3xl">
                                      {item.title === "Abroad Study" && <IconGlobe className=" h-10 w-10  text-secondary-100" />}
                                      {item.title === "Test Preparation" && <IconTestPreparation className=" h-8 w-8 text-secondary-100" />}
                                      {item.title === "Services" && <IconService className=" h-8 w-8 text-secondary-100" />}
                                    </div>
                                  </div>
                                  <div>
                                    <span className="text-gray-100 text-sm font-medium md:text-base">{el.heading}</span>
                                    <p className="text-sm text-gray-300  mt-1">{el.shortDesc}</p>
                                  </div>
                                </Link>
                              </li>
                            ))}
