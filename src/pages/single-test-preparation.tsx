...
export default function SingleTestPreparation() {
  const params = useParams();
  const [blog, setBlog] = useState<any>({});
  const [isFetching, setIsFetching] = useState(false);
  useEffect(() => {
    const fetch = async () => {
      let { data, error } = await supabase.from("TestPreparation").select("*").eq("slug", params.slug).single();
      if (error) {
      ...
      <div className=" space-y-4">
        {blog && (
          <img
            ...
            width={700}
            height={700}
            className=" h-[70vh] w-full object-cover rounded-md shadow-lg"
          />
        )}
